...
const Leaderboard = () => {
  ...
  const getRankStyle = (rank) => {
    ...
    return 'bg-(--bg-secondary) border-(--border-color)';
  };
        ...
        <div className="space-y-3">
          {leaders.map((leader) => (
            ...
            >
              ...
              {/* Avatar */}
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-linear-to-br from-primary-500 to-primary-700 flex items-center justify-center text-white font-bold text-sm sm:text-lg shrink-0">
                {leader.name.split(' ').map(n => n[0]).join('')}
              </div>

              {/* Info */}
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-(--text-primary) text-sm sm:text-base truncate">{leader.name}</h3>
                <p className="text-xs sm:text-sm text-(--text-secondary)">{leader.wins} contest wins</p>
              </div>
              ...
              <div className="text-right shrink-0">
                ...
                <p className="text-[10px] sm:text-xs text-(--text-secondary)">points</p>
              </div>
            </div>
          ))}
        </div>
